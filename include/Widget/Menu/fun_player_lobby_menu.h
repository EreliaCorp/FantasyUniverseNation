...
namespace fun
{
	namespace Menu
	{
		class PlayerLobbyMenu : public fun::Abstract::Menu, public jgl::Singleton_widget<PlayerLobbyMenu>
		{
			friend class jgl::Singleton_widget<PlayerLobbyMenu>;
		...
		private:
			...
			{
				THROW_INFORMATION("On server instanciation [PlayerLobbyMenu]");
			}

			void _on_client_instanciation()
			{
				THROW_INFORMATION("On client instanciation [PlayerLobbyMenu]");
			...
			PlayerLobbyMenu(jgl::Widget* p_parent) : Menu(p_parent)
			{
